        ...
        <div class="events-table-next-events">
            {foreach $next_events as $event}
                <div class="row event-row">
                    <div class="col-sm-8">
                        <h3 class="event-title">{$event['timestamp']|date_format: "%A, %e %B @ %l%P"}
                            : {$event['name']}</h3>
                    </div>
            ...
            <tbody>
                {foreach $events as $event}
                    <tr>
                        ...
                        <td><a href="{$event['tickets_link']}" class="btn" target="_blank"><span
                                        class="hidden-xs">Book </span>Tickets</a></td>
                    </tr>
                {/foreach}
            </tbody>
        </table>
